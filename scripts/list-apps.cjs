...
if (!config.issuerId || !config.keyId || !config.privateKey) {
  console.error('\x1b[31mError: Missing App Store Connect API credentials.\x1b[0m');
  console.error('Please configure APP_STORE_CONNECT_ISSUER_ID, APP_STORE_CONNECT_KEY_ID, and APP_STORE_CONNECT_PRIVATE_KEY_PATH in your .env file or eas.json.');
  process.exit(1);
}
...
async function main() {
  try {
    console.log('\n--- FETCHING APPS ---');
    const res = await apiRequest('GET', '/v1/apps');
    console.log(`Total apps found: ${res.data.length}`);
    res.data.forEach(app => {
      console.log(`Name: ${app.attributes.name}, ID: ${app.id}, Bundle ID: ${app.attributes.bundleId}`);
    });
  } catch (error) {
